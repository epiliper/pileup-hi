...
extern "C" {
    fn tweak_overlap_quality(
        a: *mut rust_htslib::htslib::bam1_t,
        b: *mut rust_htslib::htslib::bam1_t,
    ) -> i32;
}

pub fn tweak_overlap_qual(a: &mut Record, b: &mut Record) -> Result<(), Error> {
    unsafe {
        let ret = tweak_overlap_quality(
            a.inner_mut() as *mut _,
            b.inner_mut() as *mut _,
        );
        if ret < 0 {
            let qname = std::str::from_utf8(a.qname())?;
            return Err(Error::from(ErrorKind::MateOverlapFailed(
                qname.to_string(),
            )));
        }
    }
...
impl MapOverlaps for OverlapMap {
    fn push(&mut self, plp: Rc<RefCell<PileupAlignment>>) {
        ...
        if (r.mtid() >= 0 && (r.mtid() != r.tid()))
            || r.insert_size().abs() >= 2 * (r.seq_len() as i64)
                && r.mpos() >= r.pos() + len
        {
            return;
        }
